...
class ProgressSummaryx extends React.Component {
  ...
  CreateExcel = async () => {
    ...
    try {
      ...
      //alert("calling mystore from createexcel");
      const store = await getexceldata(this.props.clientCode);
      //alert("finished call");
      ...
      if (Array.isArray(data)) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          // Update lastRowNumber with the current item's ROWNUMBER
  ...
  CreateExcel2 = async () => {
    ...
    try {
      ...
      const response = await getexceldata(this.props.clientCode);
      ...
      data.forEach((item) => {
        ...
        // Assume these functions are correctly defined elsewhere
        renderExcelValueFieldCell(row.getCell("VALUEFIELD"), item);
        renderExcelDescriptionFieldCell(row.getCell("DESCRIPTION"), item);
      });
      ...
      // Assuming MySwal is correctly initialized
      this.MySwal.fire({
        icon: "success",
        title: "Excel File Created",
        text: `Your file ${this.props.clientCode}.xlsx has been successfully created.`,
...
const CreateExcelx = async () => {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      //alert("Excel File Created - no save yet");
      saveAs(
        new Blob([buf], {
          type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        }),
      ...
      //alert("Excel File Created - saved");
